...
- Тема: подсистема-конструктор и подсистема проведения анкетирования в АСНИ социологических данных
- Объект разработки: веб-сервис для создания опросов и сбора ответов респондентов
- Стек: React, FastAPI, PostgreSQL, Docker
- Студент: [ФИО], группа [номер], ННГУ ИИТММ, 2026
...
- АСНИ — автоматизированная система научных исследований (6 подсистем на схеме)
- В ВКР реализованы две подсистемы: конструктор анкет и проведение анкетирования
- Конструктор: исследователь собирает и публикует анкету
- Проведение: респондент отвечает по ссылке в браузере
- Общее хранилище: PostgreSQL (схема анкеты и ответы людей в одной базе)
...
- Развёртывание: Docker Compose (3 сервиса)
- survey-db — PostgreSQL 16 (анкеты, версии, сессии ответов)
- survey-api — сервер FastAPI (REST API, проверки сроков и лимитов)
- survey-frontend — сайт React + SurveyJS (редактор и страница опроса)
- Цепочка запросов: браузер → nginx → API → база данных
- Исследователь входит по логину (роли admin / researcher / student); респондент — только по ссылке, без пароля
...
- Для кого: исследователь (после авторизации в /admin/surveys)
- Визуальный редактор SurveyJS Creator: страницы, типы вопросов, вкладка «Логика»
- Автосохранение черновика (~1 с), ручное сохранение, настройка сроков и лимита ответов
- Публикация анкеты → публичная ссылка вида https://.../s/{id}
- Журнал версий: кто и когда менял схему анкеты
- Статистика по сессиям; выгрузка ответов в CSV или JSON
...
- Проблема: анкету часто правят уже во время сбора данных
- Решение: журнал версий в таблице survey_versions
- При каждом сохранении — снимок survey_json + номер версии + автор + краткий diff
- В редакторе: панель «История версий» (список, раскрыть изменения)
- Можно восстановить любую прошлую версию (с подтверждением)
- Откат не уничтожает историю: в журнале остаётся запись «восстановлено с версии N»
...
- Динамическое анкетирование: вопросы и страницы зависят от ответов респондента
- Правила задаёт исследователь в Creator (условия visibleIf, ветвление)
- Исполнение в браузере: SurveyJS Runner, без перезагрузки страницы
- Один файл survey_json — и для редактора, и для прохождения опроса
- Прогресс считается только по видимым сейчас вопросам (лишние блоки не мешают)
- Пример: «Показать вопрос 5, только если в вопросе 1 выбран ответ "да"»
...
- Страница опроса: /s/{id} — открытый доступ, без регистрации
- Шаги: загрузка анкеты → (ввод имени, если нужно) → ответы → завершение
- Сессия = одна попытка прохождения; id хранится в localStorage браузера
- Автосохранение на сервер каждые ~0,7 с (ответы, номер страницы, % заполнения)
- Можно закрыть вкладку и продолжить позже в том же браузере
- Сервер проверяет: анкета опубликована, срок не вышел, не превышен лимит ответов
...
- getAllQuestions(false) + isVisible — только вопросы, которые сейчас показаны
- На сервер уходят: answers_json, current_page, progress_pct
- Связь с динамикой: скрытый вопрос не входит в процент заполнения
...
Цель работы выполнена: реализованы и описаны подсистема-конструктор и подсистема проведения анкетирования для АСНИ.

Что сделано в прототипе:
- Веб-сервис: редактор анкет (SurveyJS Creator), публикация, публичная страница опроса /s/{id}
- Хранение в PostgreSQL: анкеты, журнал версий (survey_versions), ответы респондентов (survey_sessions)
- Развёртывание в Docker Compose (база, API, фронт с nginx)

Полный сценарий использования:
1. Исследователь создаёт анкету, настраивает логику и сроки, публикует
2. Респонденты проходят опрос по ссылке (autosave, можно продолжить в том же браузере)
3. Исследователь смотрит статистику и выгружает ответы в CSV или JSON

Главные преимущества разработки:
- Версионирование анкет: история правок, diff, восстановление любой версии без потери журнала
- Динамическое анкетирование: вопросы и страницы зависят от ответов; один survey_json для редактора и опроса

Качество и ограничения:
- E2E-тесты API в CI (сессия, autosave, завершение, экспорт)
- Ограничение MVP: черновик привязан к браузеру (localStorage); перенос на другой ПК без доработки
- В перспективе: отдельные экраны «опрос не начался» / «срок вышел», сессия по персональной ссылке
...
На этом слайде подведу итоги. Цель моей работы была сделать две части АСНИ: чтобы исследователь мог собрать анкету на экране и чтобы респондент мог пройти её по ссылке, а ответы легли в базу. Это сделано: есть рабочий прототип, он описан в пояснительной записке и поднимается в Docker.

Кратко по шагам. Сначала исследователь в конструкторе собирает анкету, при необходимости с ветвлением «показать вопрос только если…». Публикует и получает ссылку. Потом люди открывают эту ссылку, отвечают; ответы сами сохраняются на сервер, можно прерваться и вернуться в том же браузере. В конце исследователь смотрит статистику и скачивает файл.

Два момента, которые я считаю главным результатом. Первый — версионирование: при каждой правке анкеты пишется версия, видно что менялось, можно откатиться. Второй — динамическая анкета: респондент не видит лишних вопросов, логика работает в браузере из того же JSON, что и в редакторе.

Честно про ограничения: если человек с другого компьютера зашёл или очистил браузер, продолжить тот же незавершённый опрос пока нельзя. Это зафиксировано как ограничение MVP. Дальше можно доработать экраны по срокам опроса и переносимую сессию.
