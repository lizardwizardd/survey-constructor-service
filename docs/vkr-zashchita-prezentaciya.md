...
- Рабочий прототип: полный цикл от редактора до выгрузки ответов
- 1. Собрать анкету (с логикой) → 2. Опубликовать → 3. Респонденты по ссылке → 4. Статистика и CSV/JSON
- Главные результаты: версионирование схемы анкеты; динамическое прохождение (SurveyJS)
- Тесты: e2e-скрипты в CI (создание сессии, autosave, complete, export)
- Ограничение MVP: продолжить черновик можно только в том же браузере (localStorage)
